...
function App() {
  const [currentScreen, setCurrentScreen] = useState<
    'light-lab' | 'timeline' | 'browser' | 'devices' | 'node-editor' | 'tokens' | 'k1-showcase' | 'pattern-studio' | 'pattern-studio-glass' |
  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50">
        <div className="bg-[var(--k1-panel)] shadow-k1-md border border-[var(--k1-border)] rounded-lg p-2">
          <Tabs value={currentScreen} onValueChange={(v) => setCurrentScreen(v as any)}>
            <TabsList className="grid grid-cols-12 w-full">
              <TabsTrigger value="light-lab" className="gap-1.5 text-xs px-2">
